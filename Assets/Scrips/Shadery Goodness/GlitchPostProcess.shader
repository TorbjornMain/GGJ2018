Shader "Hidden/GlitchPostProcess"
{
	Properties
	{
		_MainTex ("Texture", 2D) = "white" {}
	}
	SubShader
	{
		// No culling or depth
		Cull Off ZWrite Off ZTest Always
		Tags { "Queue"="Overlay+1000" }
		Pass
		{
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			
			#include "UnityCG.cginc"
			#include "Common.cginc"

			struct appdata
			{
				float4 vertex : POSITION;
				float2 uv : TEXCOORD0;
			};

			struct v2f
			{
				float2 uv : TEXCOORD0;
				float4 vertex : SV_POSITION;
			};

			v2f vert (appdata v)
			{
				v2f o;
				o.vertex = UnityObjectToClipPos(v.vertex);
				o.uv = v.uv;
				return o;
			}
			
			sampler2D _MainTex;

			fixed4 frag(v2f i) : SV_Target
			{
				float2 processedUV = (i.uv)* float2(10, 50);
				processedUV -= frac(processedUV + float2(Random(_Time.y * i.uv.y * 20), Random(_Time.x)));
				float2 uvOff = float2(Random(dot(processedUV, processedUV) + _Time.w), Random(dot(processedUV, processedUV) + 90 + _Time.w));
				uvOff = float2(Random(_Time.w * 1000 + i.uv.y * 30), 0);
				fixed4 col = tex2D(_MainTex, i.uv);
				col += tex2D(_MainTex, i.uv + uvOff * 0.01) * 0.1f;
				col += tex2D(_MainTex, i.uv - uvOff * 0.01) * 0.1f;

				// just invert the colors
				return col;
			}
			ENDCG
		}
	}
}
